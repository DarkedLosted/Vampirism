...
function Bot:LevelInit(playerID)
    ...
    local abilityName = nil
     Bot.Hero = hero
     hero:SetRevealRadius(2400) -- vision
    	for i=1,3 do
...
function Bot:GlobalThink(  )
	-- body
	Bot:CheatWherePlayers()
	-- local path = Entities:FindByName(nil,"base_1")
	-- print(path:IsTouching(Bot.Hero))
	return 1
end
...
if Bot.Agressive then
	...
	Bot.AttackTarget=find_target()
		if Bot.RB then
			Move_Module(Bot:GetBasePositions[Bot.RB])
		end
		Move_Module(Bot:GetBasePositions[Bot.AttackTarget])
	-- if Bot.Mode == "MoveOn"  then
	-- 	Bot:TargetRich()
	-- end
	end
end
...
function Bot:Think()
	...
	--Bot:TargetRich()
		...
		if not Bot.AttackTarget then 
				local path = Entities:FindByName(nil,"path_1")
				local path1 = Entities:FindByName(nil,"path_1_1")
				local path2 = Entities:FindByName(nil,"path_1_2")

				ExecuteOrderFromTable({ UnitIndex = Bot.Hero:GetEntityIndex(), OrderType = DOTA_UNIT_ORDER_ATTACK_MOVE, Position = path:GetAbsOrigin(), Queue = true})
				ExecuteOrderFromTable({ UnitIndex = Bot.Hero:GetEntityIndex(), OrderType = DOTA_UNIT_ORDER_ATTACK_MOVE, Position = path1:GetAbsOrigin(), Queue = true})
				ExecuteOrderFromTable({ UnitIndex = Bot.Hero:GetEntityIndex(), OrderType = DOTA_UNIT_ORDER_ATTACK_MOVE, Position = path2:GetAbsOrigin(), Queue = true})
				...
				-- 	return Bot.ThinkInterval
				-- else
				-- 	local tempForward = Bot:Move(Bot:GetBasePositions()[tostring(RandomInt(1,42))])
				-- 	Bot.CurrentMoveTarget = tempForward
				-- 	Bot.AttackTarget=true
				-- end
...
function find_target()
	...
	if  Bot.RB then
		...
		Bot.RB= tostring(RandomInt(1,42))
		return false
	end
end

function Move_Module(pos)
	

	if  Bot.Hero:GetAbsOrigin().x ~= Bot.AttackTarget.x and Bot.Hero:GetAbsOrigin().y ~= Bot.AttackTarget.y then
...
function Bot:CheckReveal()
	...
	if Bot.Ability["vampire_reveal"] and Bot.Ability["vampire_reveal"]:IsFullyCastable() then
		...
		local enemyarr = FindUnitsInRadius(DOTA_TEAM_BADGUYS, BasePos, nil, vision_radius-500, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_NONE+DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
...
function Bot:CheatWherePlayers()
	...
	for i=1,42 do
		local enemyarr = FindUnitsInRadius(DOTA_TEAM_BADGUYS, BasePos[tostring(i)], nil, 1500, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_NONE+DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
		--Bot.LocationTable = {}
		if #enemyarr >10 then
